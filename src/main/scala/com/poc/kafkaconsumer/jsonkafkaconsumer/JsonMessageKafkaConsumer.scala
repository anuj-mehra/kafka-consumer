...
class JsonMessageKafkaConsumer {

  def process: (Properties, KafkaConsumerConfig) => Unit = (properties: Properties, config: KafkaConsumerConfig) => {
...
object JsonMessageKafkaConsumer{
  ...
  def apply = new JsonMessageKafkaConsumer
}
